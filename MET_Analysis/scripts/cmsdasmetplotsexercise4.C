...
void cmsdasmetplotsexercise4() {
  ...
  TCanvas *c_scale = createCanvas("scale", leg, hist_scale, "q_{T} [GeV]",
                                  "-u_{||}/q_{T}", "_i / _0");
  TCanvas *c_respara_vspt =
      createCanvas("respara_vspt", leg, hist_respara_vspt, "q_{T} [GeV]",
                   "#sigma u_{||} [GeV]", "_i / _0");
  TCanvas *c_resperp_vspt =
      createCanvas("resperp_vspt", leg, hist_resperp_vspt, "q_{T} [GeV]",
                   "#sigma u_{#perp} [GeV]", "_i / _0");
  TCanvas *c_respara_vspu =
      createCanvas("respara_vspu", leg, hist_respara_vspu, "N_{vtx}",
                   "#sigma u_{||} [GeV]", "_i / _0");
  TCanvas *c_resperp_vspu =
      createCanvas("resperp_vspu", leg, hist_resperp_vspu, "N_{vtx}",
                   "#sigma u_{#perp} [GeV]", "_i / _0");
}
...
TCanvas *createCanvas(TString cName, TLegend *leg, std::vector<TH1F *> hist,
  ...
  // return cTemp;
  cTemp->Print("../outputs/fig/output_step4_" + cName + ".pdf");
  return 0;
}
