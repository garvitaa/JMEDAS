...
void cmsdasmetplotsexercise3(TString step) {
  ...
  TFile *f_ = TFile::Open(
      "root://cmsxrootd.fnal.gov//store/user/cmsdas/2025/short_exercises/MET/cmsdas_met_exercise3.root",
      "READONLY");
  ...
  if (step == "step3_scale_pfraw") {
    ...
    TCanvas *c_scale = createCanvas(step, leg, hist_scale, "q_{T} [GeV]",
                                    "-u_{||}/q_{T}", "_i / _0");
  }
  ...
  if (step == "step3_resolution_pftype1") {
    ...
    TCanvas *c_respara_vspt =
        createCanvas(step+"_respara_vspt", leg, hist_respara_vspt, "q_{T} [GeV]",
                     "#sigma u_{||} [GeV]", "_i / _0");
    TCanvas *c_resperp_vspt =
        createCanvas(step+"_resperp_vspt", leg, hist_resperp_vspt, "q_{T} [GeV]",
                     "#sigma u_{#perp} [GeV]", "_i / _0");
    TCanvas *c_respara_vspu =
        createCanvas(step+"_respara_vspu", leg, hist_respara_vspu, "N_{vtx}",
                     "#sigma u_{||} [GeV]", "_i / _0");
    TCanvas *c_resperp_vspu =
        createCanvas(step+"_resperp_vspu", leg, hist_resperp_vspu, "N_{vtx}",
                     "#sigma u_{#perp} [GeV]", "_i / _0");
  }
}
...
TCanvas *createCanvas(TString cName, TLegend *leg, std::vector<TH1F *> hist,
  ...
  cTemp->Print("../outputs/fig/output_" + cName + ".pdf");
  return 0;
  //return cTemp;
}
