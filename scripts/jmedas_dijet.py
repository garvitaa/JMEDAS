...
for ifile in files :
    ...
    for event in events:
        ...
        ijet = 0
        dijet_j0 = ROOT.TLorentzVector()
        dijet_j1 = ROOT.TLorentzVector()        
        dijet_genj0 = ROOT.TLorentzVector()
        dijet_genj1 = ROOT.TLorentzVector()        
        for jet in jets0 :
            ...
            if jet.pt() > args.minAK4Pt and abs(jet.rapidity()) < args.maxAK4Rapidity :
                ...
                # Select leading two jets in pT
                this_jet_vec = ROOT.TLorentzVector()
                this_jet_vec.SetPtEtaPhiM(corr * uncorrJet.pt(), jet.eta(), jet.phi(), jet.mass())
                if this_jet_vec.Pt() > dijet_j0.Pt():
                    dijet_j1.SetPtEtaPhiM(dijet_j0.Pt(), dijet_j0.Eta(), dijet_j0.Phi(), dijet_j0.M())
                    dijet_j0.SetPtEtaPhiM(this_jet_vec.Pt(), this_jet_vec.Eta(), this_jet_vec.Phi(), this_jet_vec.M())
                ...
                if genJet != None:
                    ...
                    elif this_genjet_vec.Pt() > dijet_genj1.Pt():
                        dijet_genj1.SetPtEtaPhiM(this_genjet_vec.Pt(), this_genjet_vec.Eta(), this_genjet_vec.Phi(), this_genjet_vec.M())
        if dijet_j0.Pt() > 0. and dijet_j1.Pt() > 0.:
            if abs(dijet_j0.Eta() - dijet_j1.Eta()) < 1.2:
                h_dijet_mass.Fill((dijet_j0 + dijet_j1).M())
